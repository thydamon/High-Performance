...
在热路径上调用 `new`/`delete` 有以下问题，它们互相叠加，共同导致不可预测的延迟：
...
`new`/`delete` 不是简单的指针加减。一次分配的完整调用链如下：
...
分配器内部的查找过程：
- 在内部数据结构（链表、树或位图）中查找不小于请求大小的空闲块；
- 如果找到的块太大，需要切割成"已分配部分"+"剩余空闲块"；
- 更新元数据，标记该块已分配；
- **最坏情况**：整个进程堆中无合适空闲块时，调用 `brk()` 或 `mmap()` 向内核申请新内存——这是系统调用，涉及用户态到内核态切换，延迟可达数十微秒。

释放 `delete(ptr)` 时：
- 检查前后相邻块是否空闲，如果空闲则需要合并（减少碎片）；
- 把块插回空闲链表或更新位图；
- 大块释放可能触发 `munmap()` 归还给内核。

延迟波动量级：
- 最好情况（命中 free list 快速路径）：几十纳秒；
- 最坏情况（触发 `mmap` + 物理页分配）：数百微秒；
- 差异可达 **1000–10000 倍**，对低延迟系统不可接受。

**2. 锁竞争**

默认的 C++ 内存分配器（Linux 下通常是 glibc 的 ptmalloc）为了保证线程安全，内部使用互斥锁保护全局堆数据结构：

```
线程 A:                   线程 B:
  lock(堆锁)
  malloc(64)               lock(堆锁) → 阻塞等待！
    ↓ 查找空闲块             ↓ 操作系统挂起线程 B
    ↓ 切割块                 ↓ 等待线程 A 解锁...
  unlock(堆锁)
                           lock(堆锁) → 获得锁
...
多线程场景下的连锁反应：
- **线程排队串行化**：每个线程都要等前一个释放锁，多核并行优势消失；
- **上下文切换**：等待锁的线程被操作系统挂起，唤醒时又要切换回来，每次切换成本 ~1–5 µs；
- **优先级反转**：高优先级的低延迟线程可能被低优先级的普通线程持锁阻塞；
- **惊群效应**：锁释放时大量线程同时竞争。

现代分配器的改进（jemalloc 的 tcache、tcmalloc 的 per-thread cache）大幅减少了锁竞争，但无法完全消除——当 thread-local cache 用尽或归还不属于本线程的内存时，仍然要走全局锁路径。

**3. 内存碎片**

**外部碎片**：长时间运行后，堆内存变成"瑞士奶酪"：

```
已分配 | 空闲 | 已分配 | 空闲 | 已分配 | 空闲 | 已分配 | 空闲
  64     32      128     48       64     16      32      64
```

总空闲空间有 192 字节，但如果请求 100 字节的连续块，找不到任何一个足够大的空闲块。分配器不得不问内核要更多内存，进程内存占用越来越大，查找合适空闲块的耗时也越来越长。

**内部碎片**：分配器按固定大小类别管理内存（如 16、32、64、128 字节等级别），请求 65 字节可能分配 128 字节的块，浪费 63 字节。

低延迟的后果：随着碎片化加剧，性能随时间**逐步退化**——之前 50 ns 的分配现在需要 200 ns，之前稳定的 5 µs 处理时间偶尔飙到 50 µs。很多低延迟系统定时重启，部分原因就是内存碎片。

**4. 缓存不友好**

`new` 返回的内存地址不可控，无法预测其缓存状态：

| 情况 | 说明 |
|---|---|
| 冷缓存 | 新分配的内存大概率不在任何缓存中，第一次访问必须从主内存加载（100–300 个时钟周期） |
| 非连续 | 多次 `new` 分配的对象在内存中分散存放，遍历时每个都可能是 cache miss |
| 跨缓存行 | 分配的大小跨越多个 64 字节缓存行，读一个对象可能要加载多条缓存行 |

对比：内存池分配的对象在连续内存中，一个缓存行可以覆盖多个对象，遍历几乎全是 cache hit。

**5. TLB 压力**

TLB（Translation Lookaside Buffer）是 CPU 内部硬件缓存，存储"虚拟地址 → 物理地址"的映射关系。CPU 访问数据前，需要先查到虚拟地址对应的物理地址。

频繁分配/释放对 TLB 的影响：

| 行为 | 后果 |
|---|---|
| 频繁 `mmap`/`munmap` | 页表被修改，对应的 TLB 条目全部失效 |
| 碎片导致频繁申请新页 | 页表频繁修改，TLB 刷新频繁 |
| 对象跨多个 4KB 页 | 访问一个对象就要占多个 TLB 条目，增加 miss 概率 |
...
| L1 TLB 命中 | ~1 个周期 |
| L2 TLB 命中 | ~5–10 个周期 |
| 页表遍历（PTE 在 L1 缓存） | ~20–30 个周期 |
| 页表遍历（PTE 在 L3/内存） | 100–300+ 个周期 |

频繁的 TLB miss 叠加已经很严重的 cache miss，会让延迟进一步恶化。

**一句话总结**

> `new`/`delete` 的五个问题层层叠加：非确定性延迟由查找和系统调用引起，锁竞争在多线程下放大，碎片随时间恶化一切，缓存和 TLB 未命中让访问成本再翻几倍。内存池正是针对这五个问题，分别给出可预测的解决方案。
